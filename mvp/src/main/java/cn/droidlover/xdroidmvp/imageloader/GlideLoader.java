...
public class GlideLoader implements ILoader {
    ...
    private void load(DrawableTypeRequest request, ImageView target, Options options) {
        ...
        if(options.radius != Options.RES_NONE){
            wrapScaleType(request, options).diskCacheStrategy(DiskCacheStrategy.SOURCE).crossFade().transform(new GlideRoundTransform(mContext,options.radius)).into(target);
        }else{
            wrapScaleType(request, options).diskCacheStrategy(DiskCacheStrategy.SOURCE).crossFade().transform(new GlideCircleTransform(mContext)).into(target);
        }
    }
