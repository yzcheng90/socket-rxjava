...
import com.sukeintel.iexhibition.mobile.app.works.mvp.adapter.power.LampAdapter;
...
import com.sukeintel.iexhibition.mobile.app.works.mvp.present.power.LampPresent;
...
public class LampListActivity extends BaseActivity<LampPresent> {
    ...
    public SimpleRecAdapter getAdapter() {
        if (adapter == null) {
            adapter = new LampAdapter(context);
            adapter.setRecItemClick(new RecyclerItemCallback<ResourceEntity, LampAdapter.ViewHolder>() {
                @Override
                public void onItemClick(int position, ResourceEntity model, int tag, LampAdapter.ViewHolder holder) {
    ...
    public void reLoad(){
        getmPresenter().getResourceListByType(ResourceEntity.ResourceType.SwitchResource);
    }


    @Override
    public LampPresent newP() {
        return new LampPresent();
    }
